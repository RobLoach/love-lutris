...
function lutro.load()
  ...
  font = lutro.graphics.newImageFont('resources/font.png', ' abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.,!?-+/')
	...
	lutro.graphics.setBackgroundColor(190, 79, 68)
...
function lutro.draw()
  ...
  lutro.graphics.draw(logo, 900 - 256 - lineHeight, lineHeight)
  ...
  for i, game in ipairs(games) do
    ...
    if currentSelection == i then
      lutro.graphics.rectangle('fill', 90, y + lineHeight / 4, lineHeight / 3, lineHeight / 3)
    end
    -- Display the menu item
    lutro.graphics.print(game.name, 100, y)
    y = y + lineHeight
  end
end
